...
export function ContractsTab({ properties, units, tenants, onRefresh }: Props) {
  ...
  // Kündigungsdatum automatisch berechnen (Eingang + 3 Monate zum Monatsende)
  const handleNoticeDateChange = (dateStr: string) => {
    setCancellationReceivedAt(dateStr);
    if (!dateStr) return;

    const noticeDate = new Date(dateStr);
    // Standard 3 Monate Kündigungsfrist
    const targetDate = new Date(noticeDate.getFullYear(), noticeDate.getMonth() + 4, 0); // Letzter Tag des 3. Folgemonats
  ...
  const confirmArchive = async () => {
    ...
    if (error) {
      ...
      // Auch Auszugsdatum beim Mieter aktualisieren
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block font-medium text-slate-700 mb-1">
                  Vertragsende / Kündigungsdatum (Standard: 3 Monate Frist)
                </label>
                <input
                  type="date"
                  value={endDate}
              ...
              >
                <div>
                  ...
                  <p className="text-slate-400 text-[10px] mt-0.5">
                    Mietbeginn: {contract.start_date || contract.created_at?.split("T")[0]}
                  </p>
                </div>
